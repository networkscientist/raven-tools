...
def write_rvt_meteo(pet: bool = False, start_date: object = '2008-01-01', end_date: object = '2008-12-31') -> object:
    ...
    if pet:
        ...
        filename = f"{writePath}forcings/{rvt_filename}.rvt"
    ...
    df_meteo_new = df_meteo[['time', 'TEMP_AVE', 'TEMP_MIN', 'TEMP_MAX', 'PRECIP']]
    df_meteo_new['time'] = pd.to_datetime(df_meteo_new['time'], format="%Y%m%d%H")
    df_meteo_new = df_meteo_new.sort_values(by='time', ascending=True).dropna()
    df_meteo_new = subset_dataframe_time(df_meteo_new, start_date, end_date)
    with open(filename, 'w') as f:
        print(filename)
        f.write(f":MultiData\n{start_date}\t0:00:00\t0.041666667\t{len(df_meteo_new)}\n")
        if pet:
            f.write(":Parameters\tTEMP_AVE\tTEMP_MIN\tTEMP_MAX\n")
            f.write(":Units\tC\tC\tC\n")
            df_as_string = df_meteo_new.to_string(justify="right", header=False, index=False,
                                              columns=['TEMP_AVE', 'TEMP_MIN', 'TEMP_MAX'])
        else:
            f.write(":Parameters\tPRECIP\tTEMP_AVE\tTEMP_MIN\tTEMP_MAX\n")
            f.write(":Units\tmm/d\tC\tC\tC\n")
            df_as_string = df_meteo_new.to_string(justify="right",header=False, index=False, columns=['PRECIP', 'TEMP_AVE', 'TEMP_MIN',
                                                                                      'TEMP_MAX'])
...
write_rvt_meteo(pet=False, start_date='2008-01-01', end_date='2008-12-31')
