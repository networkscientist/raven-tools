...
with open("raven_tools/default_params.yaml", "r") as f:
...
def generate_template(csv_file, model_type=model_type, params=default_params, param_or_name="params"):
...
def write_rvx(model_dir=model_dir,
    ...
    if template_type == "Ostrich":
        ...
        template_sections = generate_template(model_type=model_type, csv_file=csv_file, params=params,
                                              param_or_name="names")
        logger.debug(f"Wrote .{rvx_type}.tpl template sections generated by generate_template() to dict template_sections")
    logger.debug(f"Trying to write to file {file_path}")
    with open(file_path, 'w') as ff:
        # print("Header:")
        ff.writelines(f"{line}{newline}" for line in header)
